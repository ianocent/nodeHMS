...
## 2026-08-15 sesi 5: report parity lengkap — account/sales/frontoffice batch (kecommit b1ec0b2 + 2b92243, KEPUSH)

### Commit 1 (b1ec0b2) — approval matrix + service scheduler + email template + 19 report handler
- approval-matrix.controller.ts baru (parity ApprovalMatrixController): role_ids Json disimpan Number (bukan BigInt[]), laravelPaging tanpa null keys; model approval_matrices di-add ke prisma/schema.prisma + npx prisma generate.
- service-scheduler.controller.ts baru (parity ServiceSchedulerController): housekeeper_history tidak punya relasi rooms di Prisma -> room_id manual + rooms -> room_type_id -> types group 'floor' untuk floors; paginate tanpa prev/next null.
- content.controller.ts sendMailTemplate: include reservations+room_types (reservations TIDAK punya relasi rooms), room via prisma.rooms.findUnique manual, field child (bukan children), company relasi company_profiles_folios_company_profile_idTocompany_profiles. Email = LOG ONLY (tanpa nodemailer).
- 19 report handler baru: room-status-report, block-rooms-report, room-change-history, cancellation-listing, birthday-report, roomtype-utilization, inclusive-items, rate-code-analysis, vacant-and-dirty-rooms, daily-room-forecast (raw \), breakfast-report, room-revenue-breakdown, comission-for-booking(+company), tax-breakdown-summary, in-house-folio-bal-history, transaction-report-by-staff, daily-revenue-report + tax-breakdown-detail (getAsyncJobReport: insert tabel requests, respond "Report is being generated. Please check back later." — parity Laravel async job pattern).
- Alias handler: batch/after-night-audit/in-house-foliobal, batch/before-night-audit/before-in-house-foliobal, account/on-resv-bal.

### Commit 2 (2b92243) — 14 report handler tersisa, report coverage 100%
- account/daily-sales-report (getAccountDailySalesReport): MTD/LYTD room revenue via transaction_breakdowns + code_posts/code_billings filter (room revenue/banquet/breakfast/dine in/room service/minimart/fb/other), ARR/RevPAR/occupancy + variance vs last year, dailyStats/mtdStats/forecastStats, actualBalance/mtdBalance (variable/fixed cost via post_code_budgets by month/year), kurs JPY convert, roomSales, dailyRevenue/mtdRevenue.
- account/daily-statistic-report (Daily Flash parity DailyFlashReportController): totalAvailable/Blocked(room_status 3,4)/Occupied/Sold/HouseUse/Complimentary/Saleable/Vacant/WalkIn/DayUse/InHouseGuests/ARR/ARRIncBF/RevPAR/3 jenis occupancy/doubleOccupancy + roomTypeSales per tipe + mtdLastMonth/ytdActual.
- frontoffice 7: free-of-charge-detail (group HSE/COMP per room type, summary CRT/CRD/BRD), reservations-by-staff (group by created_by + cancelled), room-type-detailed (ASYNC JOB pattern), in-house-guest-listing (status [0,1,4], first_name != POS, roomTypeSummary), room-type-monthly (per room type fit/git arr/dep/sty/revenue + occupied via status 0/houseUse/comp, block room_status in (4,3), virtual room type exclude via model_has_types group room-type-grouping), same-day-check-out-check-in (guest dgn prev folio check_out == check_in), transaction-by-staff-report (group by type, card_name/last_digit, description parity, signed total).
- sales-marketing 5: all-companies-room-revenue (per company folio rows + roomNights per tanggal via reservations count, ANR/AGR, grand total; parent GIT -> child folio), market-segmentation (types group market-segment-1..4 -> company: nationality billing_country, ANR/AGR), nationality-statistics-detailed (per nationality id folio/guest, rateInclusives cost via code_items calculator Adult/Child/Room, room names #-join), staff-sales-summary (company staff_in_charge -> user, projection/actual nights+revenue, listFolio), room-occupancy-chart (per tanggal range, fit/git arr/dep/sty/revenue, weekly Monday-start avg, monthly avg, grandTotal).
- Fakta Prisma baru: model_has_code_items @@ignore -> \ wajib; model_has_types punya relasi types (morph lookup terbalik); room_types name TIDAK support mode:insensitive (filter JS); companies.staff_in_charge String (bukan BigInt); folios.is_investor TIDAK ada (summary 0).
