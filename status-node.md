...
## 2026-08-14 floor plan + room availability + rate edit master + room type scope (UNCOMMITTED)
- **Room Statistic (Floor Plan) crash root cause**: GET /cms/room-statistic returned buildingGrouped tree as data + master tanpa building/templates + meta keys DROPPED (success() hanya emit whitelisted meta keys) -> frontend room-statistic/index.tsx: w?.map_id.replaceAll crash (data rows no map_id). Rewritten to RoomStatisticController.index parity: rows = Room::formatData() (map_id ucfirst, floor/building {value,label}, room_status/room_status_color/maid_status/room_clean_status_color + color codes, room_type_id {value,label}, room_type_grouping, status, property, room_id, room_configuration, folio {folio_id,url,folio_status_color_code,folio_number,folio_status}); reservation = business date single day, folio not cancelled(2)/checked-out(1); building tree from types group 'template-floor-plan' (mht links -> building/floor names, layout = slug(description), code_image = text); master + room_type_groups (group 'room-type-grouping') + templates. Filters from GET ?key_X=1,2 AND POST body keys room_types_1:true (Laravel srcstr parity).
- **Room Availability "Not Data"**: node roomAvailability hanya query room_availabilities (kosong). Rewritten to Laravel StatisticController.roomAvailability parity: grid rooms x dates (business date +7 hari or date_from/date_to), table = Unit(is_link)/Room Type/Room Status(is_drag)/Maid Status(is_drag) + per-date select columns (vacant/blocked), rows room_status/maid_status colored labels, cells per date = reservation (colspan by folio group, getColorReservation, HTML tooltip link) > blocked (room_availabilities uniqueCode colspan) > OOO (work_orders end_date null, colspan = diffDate or diff to end_date) > vacant (href Change ?parent=55&add=1&data=room&date=). Filters room_conf/room_type/room_type_group. master status_rooms/room_types/room_configurations/room_type_groups.
- **Rate edit master meta**: rate.controller.ts edit() success(res, result, 'Success') tanpa meta -> datauser?.master?.code_posts undefined -> Edit Rate dropdowns No Options (same bug class as Tab Rate). Fixed: success(res, result, 'Success', 200, { master }); master dihapus dari result. (create + barRateCreate sudah fixed di fd1503c.)
- **Room type property scope**: rate create/edit/barRateCreate room_types query tanpa property_id -> Room Type list semua property. Fixed: property_id: propertyId (Laravel RoomType::onlyActive parity: status 1).
